...
public class OperatorBlock extends TextBlock {
    ...
    public void onDraw(Canvas c, Paint paint, float x, float y, float offsetX, float offsetY) {
        y+=+0.1f*textSize;//shift operator down a little bit
        paint.setStrokeWidth(2);
        switch (text){
            case "+":
                c.drawLine(x + getInnerWidth()*0.1f, y + getInnerHeight()/2, x + getInnerWidth()*0.9f, y + getInnerHeight()/2, paint);
                c.drawLine(x + getInnerWidth()/2, y + getInnerHeight()/2 - getInnerWidth()*0.4f, x + getInnerWidth()/2, y + getInnerHeight()/2 + getInnerWidth()*0.4f, paint);
                break;
            case "-":
